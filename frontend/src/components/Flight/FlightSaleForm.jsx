...
export default function FlightSaleForm({ initialScan, onBack }) {
  ...
  const [passengers, setPassengers] = useState(() =>
    initialScan ? paxRowsFromScan(initialScan) : [{ ...EMPTY_PAX }]
  )
  ...
  const [segments, setSegments] = useState(() =>
    initialScan ? segmentsFromScan(initialScan) : [{ ...EMPTY_SEGMENT }]
  )
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const res = await fetch('http://127.0.0.1:8000/api/flights/save', {
        method: 'POST',
